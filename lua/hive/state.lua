...
--
-- Interaction types:
--   "chat"   → tracked in self.parents[bufnr] (Hive.ParentState)
--   "inline" → tracked in self.inline (Hive.InlineState)
--
-- Adding a future interaction type (e.g. "background"):
--   1. Add a Hive.<Type>State class and field on StateManager
...
local function _debug(msg)
  require("hive.debug").log("state", msg)
end
...
---@field subagents table<number, Hive.SubagentInfo>
...
---@class Hive.StateView
---@field parents table<number, table>
---@field active_parent_bufnr? number
---@field inline table

---@class Hive.StateManager
---@field config table
---@field parents table<number, Hive.ParentState>
---@field inline Hive.InlineState
...
---@return Hive.StateView
function StateManager:get_view()
  local ok, hierarchy = pcall(require, "hive.agents.hierarchy")
