...
function M.setup(config)
  ...
  local markdown = require("hive.agents.markdown")
...
function M._setup_keymap()
  ...
  local switch_modes = hive_config.keymap_modes("agent_switch")
  ...
  local cycle_modes = hive_config.keymap_modes("agent_cycle")
  if cycle_modes then
...
function M._setup_navigation()
  ...
  local prunable_viewer = require("hive.prune.viewer")
...
function M._setup_debug_keymap()
  ...
  chat_keymaps["debug"].callback = function(chat)
    ...
    if insert_idx then
      ...
      local small = vim.g.HIVE_SMALL_MODEL or (M._config and M._config.small_model)
      local big = vim.g.HIVE_BIG_MODEL or (M._config and M._config.big_model)
...
function M._setup_agent_manager_keymap()
  ...
  local modes = hive_config.keymap_modes("agent_manager")
  ...
  chat_keymaps["agent_manager"] = {
    ...
    callback = function(_chat)
      local agent_manager = require("hive.agent_manager")
...
function M._setup_model_picker()
  local model_picker = require("hive.tools.subagent.model_picker")
  model_picker.setup()
end

---@private
function M._setup_keymap_help()
  local keymap_help = require("hive.keymap_help")
  keymap_help.setup()
end

---Register tools (task, ask_user, skill) globally
---@private
function M._register_tools()
  ...
  local task_tool = require("hive.tools.task")
  chat_tools["task"] = {
    callback = function()
      return task_tool
    end,
  ...
  local ask_user_tool = require("hive.tools.ask_user")
  chat_tools["ask_user"] = {
    callback = function()
  ...
  if skills.has_skills() then
    local skill_tool = require("hive.tools.skill")
    chat_tools["skill"] = {
      callback = function()
        return skill_tool
      end,
  ...
  local list_dir_tool = require("hive.tools.list_directory")
  ...
  local consult_tool = require("hive.tools.consult")
  chat_tools["consult"] = {
    callback = function()
      return consult_tool
    end,
  ...
  local cmd_runner_tool = require("hive.tools.cmd_runner")
...
---@param filter_type? Hive.AgentType Filter by type ("agent" or "subagent")
---@return string[]
...
function M.create_chat(agent_name)
  ...
  if not agent then
    notify(fmt("Agent '%s' not found", agent_name), vim.log.levels.WARN)
    return nil
  end
  ...
  if not chat then
    notify("Failed to create chat", vim.log.levels.ERROR)
    return nil
  end
...
function M.activate(agent_name, chat, opts)
  ...
  local agent = M._agents[agent_name]
  if not agent then
    if not opts.silent then notify(fmt("Agent '%s' not found", agent_name), vim.log.levels.WARN) end
    return false
  end

  if not chat then
    if not opts.silent then notify("No chat provided to activate agent", vim.log.levels.WARN) end
    return false
  end

  local current_agent = M._chat_agents[chat.bufnr]
  if current_agent == agent_name then
    if not opts.silent then notify(fmt("Agent '%s' already active", agent_name), vim.log.levels.INFO) end
    return true
  end
  ...
  if not opts.silent and not is_subagent then notify(fmt("Agent: %s", agent_name), vim.log.levels.INFO) end
  ...
  if tools_config.groups and tools_config.groups[group_name] then
    ...
    for _, tool_name in ipairs(agent.tools) do
      ...
      if tool_config then
        ...
        local hive_tool = M._get_tool(tool_name)
        if hive_tool then
          if has_new_api then
            chat.tool_registry:add(tool_name, { config = hive_tool, visible = false })
          else
            chat.tool_registry:add(tool_name, hive_tool, { visible = false })
          end
        end
      end
      ::continue::
...
---@return table|nil
---@private
function M._get_tool(tool_name)
  local ok, tools = pcall(require, "hive.tools")
...
function M.deactivate(chat)
  ...
  notify("Agent deactivated", vim.log.levels.INFO)
end
