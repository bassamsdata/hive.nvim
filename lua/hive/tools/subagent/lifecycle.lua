...
function M.get_adapter_params(args)
  ...
  if model then
    ...
    if models.is_expensive_model(model.adapter, model.model) then
      log:warn(
        "[Lifecycle] Expensive model configured for %s_model: %s/%s. Consider setting a cheaper model via :Hive model %s <spec>",
        model_type,
        model.adapter,
        model.model,
        model_type
      ...
      vim.schedule(function()
        notify(
          fmt(
            "⚠️ Subagent using expensive model: %s/%s\nSet a cheaper model: :Hive model %s <adapter/model>",
            model.adapter,
            model.model,
            model_type
          ),
  ...
  if parent_adapter then
    ...
    if adapter_name then
      ...
      if models.is_expensive_model(adapter_name, model_name) then
        log:warn(
          "[Lifecycle] Expensive model detected for %s_model: %s/%s. Consider setting a cheaper model via :Hive model %s <spec>",
          model_type,
          adapter_name,
...
function M.create_child_chat(args)
  ...
  local log = require("codecompanion.utils.log")
  log:debug(
    "[Hive lifecycle] create_child_chat: model_type=%s, params=%s",
    args.model_type or "nil",
    vim.inspect(params)
  )
  ...
  if child_chat then
    log:debug(
      "[Hive lifecycle] child_chat created: adapter=%s, model=%s, url=%s",
      child_chat.adapter and child_chat.adapter.name or "nil",
      child_chat.adapter
          and child_chat.adapter.schema
          and child_chat.adapter.schema.model
          and child_chat.adapter.schema.model.default
        or "nil",
      child_chat.adapter and child_chat.adapter.url or "nil"
    )
  end
