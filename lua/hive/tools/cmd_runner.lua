...
local subagent_utils = require("hive.tools.subagent.utils")
...
---@class CodeCompanion.Tool.HiveCmdRunner: CodeCompanion.Tools.Tool
return {
  name = "cmd_runner",
  cmds = {
    compat.cmds(function(tools, args, opts)
      ...
      local function cleanup()
        ...
        if status_ns and chat_bufnr then
          local status = require("hive.tools.subagent.status")
...
- This tool is execution-only — no explanation inside the command.
  ...
  handlers = {
    ...
    ---@param self CodeCompanion.Tool.HiveCmdRunner
