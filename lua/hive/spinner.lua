...
function Spinner:_resolve_frames(frames)
  if type(frames) == "string" then
    ...
    notify(string.format("Spinner: Unknown preset '%s', using default", frames), vim.log.levels.WARN)
  ...
  if type(frames) ~= "table" or vim.tbl_isempty(frames) then
    notify("Spinner: Invalid frames config, using default", vim.log.levels.WARN)
    return CONSTANTS.SPINNER_FRAMES.braille
  end
...
function M.setup(user_config)
  ...
  api.nvim_create_autocmd("User", {
    pattern = {
      "HiveSubagentStarted",
      "HiveSubagentProgress",
      "HiveSubagentCompleted",
    ...
    callback = function(args)
      ...
      elseif args.match == "HiveSubagentStarted" then
        ...
        debug(
          string.format(
            "event: HiveSubagentStarted -> on_subagent_started(parent=%s, child=%s)",
            args.data and args.data.parent_bufnr or "nil",
            args.data and args.data.child_bufnr or "nil"
          )
        )
      elseif args.match == "HiveSubagentProgress" then
        ...
        debug(
          string.format(
            "event: HiveSubagentProgress -> on_subagent_progress(parent=%s, child=%s, tools=%s)",
            args.data and args.data.parent_bufnr or "nil",
            args.data and args.data.child_bufnr or "nil",
            args.data and args.data.tool_count or "0"
          )
        )
      elseif args.match == "HiveSubagentCompleted" then
        ...
        debug(
          string.format(
            "event: HiveSubagentCompleted -> on_subagent_completed(parent=%s, child=%s, status=%s)",
            args.data and args.data.parent_bufnr or "nil",
            args.data and args.data.child_bufnr or "nil",
