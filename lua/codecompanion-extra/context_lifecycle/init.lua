...
local ThresholdEvaluator = require("codecompanion-extra.context_lifecycle.thresholds")
local CompactionEngine = require("codecompanion-extra.context_lifecycle.compaction")

local function _debug(msg)
  require("codecompanion-extra.debug").log("context_lifecycle", msg)
end
...
function ContextLifecycleManager:setup_events()
  ...
  self._aug_id = api.nvim_create_augroup("CCExtraContextLifecycle", { clear = true })
...
function ContextLifecycleManager:_run_compaction(chat, eval)
  ...
  if self.config.notify then
    vim.schedule(function()
      if is_reset then
        ...
        vim.notify(string.format("Compacting context (%.0f%% used)...", eval.percentage), vim.log.levels.INFO)
      end
    end)
  end
  ...
  self:_call_llm_for_summary(chat, compaction_messages, function(summary, err)
    if err then
      ...
      vim.schedule(function()
        vim.notify("Compaction failed: " .. tostring(err), vim.log.levels.ERROR)
        chat:submit()
      end)
      return
    end
    ...
    if success then
      ...
      local prune_ok, pruning = pcall(require, "codecompanion-extra.prune.context_pruning")
      if prune_ok then
