...
local function _collect_keymaps()
  ...
  local name_set = {}
  for _, name in ipairs(EXTRA_KEYMAP_NAMES) do
    name_set[name] = true
  end
...
local function _build_content()
  ...
  if #entries == 0 then return { " No extra keymaps registered" }, {} end

  local lines = {}
  local highlights = {}

  ins(lines, " CodeCompanion Extra Keymaps")
...
function M.setup()
  ...
  local modes = extra_config.keymap_modes("extra_keymap_help")
  if not modes then return end

  keymaps["extra_keymap_help"] = {
    modes = modes,
    index = 70,
    callback = function()
      M.open()
    end,
    description = "[Help] Extra keymap reference",
  }
end
