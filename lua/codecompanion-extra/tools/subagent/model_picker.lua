...
-- and assign the result to vim.g.EXTRA_SMALL_MODEL or vim.g.EXTRA_BIG_MODEL
...
local function select_and_apply(model_string)
  vim.ui.select({ "small (for tasks)", "big (for consultants)", "both" }, {
    ...
    if choice:match("^small") or choice == "both" then
      vim.g.EXTRA_SMALL_MODEL = model_string
      vim.notify(fmt("Small model \u{2192} %s", model_string), vim.log.levels.INFO)
    end
    if choice:match("^big") or choice == "both" then
      vim.g.EXTRA_BIG_MODEL = model_string
      vim.notify(fmt("Big model \u{2192} %s", model_string), vim.log.levels.INFO)
    end
  end)
end
...
function M.callback(chat)
  ...
  if not change_adapter then
    vim.notify("Could not load codecompanion adapter module", vim.log.levels.ERROR)
    return
  end
  ...
  vim.ui.select(adapters_list, {
    ...
    format_item = function(item)
      local small = vim.g.EXTRA_SMALL_MODEL or ""
      local big = vim.g.EXTRA_BIG_MODEL or ""
      local indicators = {}
...
function M.get_summary()
  local small = vim.g.EXTRA_SMALL_MODEL
  local big = vim.g.EXTRA_BIG_MODEL
  local parts = {}
