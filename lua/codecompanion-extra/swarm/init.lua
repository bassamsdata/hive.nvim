-- Swarm System for CodeCompanion Extra
-- Orchestrates multiple specialized agents working in parallel on shared tasks
--
-- Usage:
--   local swarm = require("codecompanion-extra.swarm")
--
...
local function get_session_mod()
  if not _session_mod then _session_mod = require("codecompanion-extra.swarm.session") end
  return _session_mod
end

local function get_agent_mod()
  if not _agent_mod then _agent_mod = require("codecompanion-extra.swarm.agent") end
  return _agent_mod
end

local function get_orchestrator_mod()
  if not _orchestrator_mod then _orchestrator_mod = require("codecompanion-extra.swarm.orchestrator") end
  return _orchestrator_mod
end

local function get_manager_tool_mod()
  if not _manager_tool_mod then _manager_tool_mod = require("codecompanion-extra.swarm.tools.manager") end
  return _manager_tool_mod
end

local function get_worker_tools_mod()
  if not _worker_tools_mod then _worker_tools_mod = require("codecompanion-extra.swarm.tools.worker") end
...
function M.register_tools()
  ...
  -- Manager tool
  tools.swarm = {
    callback = "codecompanion-extra.swarm.tools.manager",
...
function M.info()
  return {
    name = "codecompanion-extra.swarm",
    version = "0.1.0",
