...
function SwarmOrchestrator:_spawn_agents()
  local agent_mod = require("codecompanion-extra.swarm.agent")
...
function SwarmOrchestrator:_configure_agent_chat(agent)
  ...
  local system_prompt = agent:build_system_prompt()
  agent.chat:add_message({
    role = cc_config.constants.SYSTEM_ROLE,
    content = system_prompt,
  }, { visible = false })

  local all_tools = agent:get_all_tools()
  if agent.chat.tool_registry then
    local worker_tools = require("codecompanion-extra.swarm.tools.worker")

    for _, tool_name in ipairs(all_tools) do
      local worker_tool = worker_tools.get(tool_name)
      if worker_tool then
        agent.chat.tool_registry:add(tool_name, worker_tool)
      else
        local tool_config = cc_config.interactions
          and cc_config.interactions.chat
          and cc_config.interactions.chat.tools
          and cc_config.interactions.chat.tools[tool_name]

        if tool_config then agent.chat.tool_registry:add(tool_name, tool_config) end
      end
    end
  end
end
